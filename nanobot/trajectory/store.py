"""SQLite persistence and chart queries for LLM call trajectories."""
...
class TrajectoryStore:
    ...
    def _connect(self) -> sqlite3.Connection:
        ...
        connection.create_function("trajectory_local_day", 2, self._local_day, deterministic=True)
    ...
    def _daily_rows(
        ...
        *,
        ...
        query = f"""
            SELECT trajectory_local_day(started_at_ms, ?) AS date, source,
                   {_AGGREGATE_SQL}
            FROM llm_calls
            WHERE started_at_ms >= ? AND started_at_ms < ?
    ...
    def record_many(self, calls: Iterable[LLMCall]) -> None:
        for call in calls:
            self.record(call)
